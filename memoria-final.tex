...
Dado que los dispositivos pueden detectar señales WIFI y BT, tanto del personal de la universidad y los estudiantes, dos de ellos se ha colocado muy cerca del acceso principal tratando de distinguir el personal (62) de los estudiantes (52). La puerta del parking (42) sólo puede ser utilizado por el personal.
...
\caption{Diferentes ratios entre la los datos recolectados por la DGT y por MOBYWIT en el nodo 1010.}
\label{tab:ratiosDGT}
\end{table}
...
Como muestran los resultados, ETS, ARIMA y Theta se comportan claramente mejor que el método de control. Las diferencias entre los tres métodos considerados son casi inexistentes. En cualquier caso, ETS produce los valores más bajos en cuatro de las medidas que se están utilizando. Por estas razones, se podría probablemente considerar como el mejor candidato para ser elegido como el método preferido para pronosticar este tipo de series de tiempo. Figura \ref{fig:forecasted-values} representa los valores pronosticados por ETS comparándolos con los esperados.
...
\begin{figure}[!ht]
    \begin{center}
        ...
        \caption{Número esperado vs número predicho de coches en el nodo de 1011, según los registros de MOBYWIT. Los valores predichos se han obtenido utilizando el Exponential smoothing state space model (ETS). }
...
\textit{Justificación del presupuesto:} En este paquete se ha utilizado el presupuesto destinado a ``Otros gastos'' como el uso de computación en nube (en Loggly o Microsoft Azure). Además, parte de la partida de ``Aparatos y equipos'' que estaba destinada a la colocación de los dispositivos (como la instalación de los dispositivos en la ETSIIT).
...
%***CAMBIAR POR PROYECTO Y ENLAZAR BIEN
...
El sistema aún tiene algunas pegas o puntos débiles que pueden mejorarse, como evitar el solapamiento entre direcciones en dos diferentes nodos (utilizando antenas unidireccionales o algún tipo de filtro).

Otro punto de mejora puede ser el uso de técnicas de predicción de series temporales para completar `agujeros' en los eventos detectados, por ejemplo, cuando un dispositivo se desconecta.

***TERMINA Y PON LAS COSAS EMPRESARIALES Y QUE SE SIGUEN RECOGIENDO DATOS

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  ACKNOWLEDGEMENTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section*{Agradecimientos}

Queremos agradecer personalmente al personal de la DGT Pedro Tomás Martínez y Ramón González (Sede de la DGT), y Alberto Serrano, Silvia Cabo, Rafael Castro y Adolfo Santos (Centro de Gestión del Tráfico del Sureste). También a los miembros de la UTE-ACISA, Jose Antonio Arrese, Javier Segovia, Jesús Rodríguez, y especialmente a Miguel Ángel León y a Miguel ***PONER APELLIDO***, ya que sin su colaboración este proyecto no podría haber sido llevado a cabo.
